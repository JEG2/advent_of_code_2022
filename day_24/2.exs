...
Enum.reduce([exit, entrance, exit], {0, entrance}, fn {goal_x, goal_y} = goal,
                                                      {start_minute, start_xy} ->
  path =
    {:gb_sets.singleton({start_minute, [start_xy]}), MapSet.new(), %{}}
    |> Stream.iterate(fn {paths, seen, blizzards_by_minute} ->
      {{_f, [{x, y} | _rest] = previous}, paths} = :gb_sets.take_smallest(paths)
      minute = start_minute + length(previous)
    ...
    |> Enum.find(fn {paths, _seen, _blizzards_by_minute} ->
      paths |> :gb_sets.smallest() |> elem(1) |> hd() == goal
    end)
    |> elem(0)
    |> :gb_sets.smallest()
    |> elem(1)

  {start_minute + length(path) - 1, hd(path)}
end)
|> elem(0)
|> IO.inspect()
